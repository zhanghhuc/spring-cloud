...
import com.zssq.relation.vo.GetPortalFrontHotListVO;
import com.zssq.relation.vo.GetPortalHotListVO;
import com.zssq.relation.vo.GetTeamHideHotListVO;
...
public class RelationHotController {
	...
	public ResultJSON getTeamShowHotList(@RequireValid GetTeamShowHotListVO param) throws BusinessException {
		...
		try {
			//方法进入
			log.info("RelationHotController.getTeamShowHotList:查询班组展示中热点列表");
			...
			if (pageBean == null) {
				log.error("RelationHotController.getTeamShowHotList：查询班组展示中热点列表");
...
//					}
			...
			//方法出去
			log.info("RelationHotController.getTeamShowHotList:查询班组展示中热点列表成功");
		} catch (BusinessException e) {
			throw e;
		} catch (Exception e) {
			log.error("RelationHotController.getTeamShowHotList", e);
	 ...
	 * @Description: 查询班组隐藏热点列表
	 * @param param
	 * @return
	 * @throws BusinessException    参数  
	 * @return: ResultJSON    返回类型
	 */
	@RequestMapping(value="getTeamHideHotList",method=RequestMethod.POST)
	@ResponseBody
	public ResultJSON getTeamHideHotList(@RequireValid GetTeamHideHotListVO param) throws BusinessException {
		//返回值
		ResultJSON resultJSON = null;
		
		try {
			//方法进入
			log.info("RelationHotController.getTeamHideHotList:查询班组隐藏热点列表");
			...
			RelationHotVO relationHotVO = new RelationHotVO();
			relationHotVO.setTeamCode(StringTools.formatToString(teamCode));
			relationHotVO.setHotStatus(RelationConstants.RELATION_HOT_STATUS_HIDE);	//service层用于判断展示隐藏热点
			relationHotVO.setQueryTime(StringTools.safeToLong(queryTime));
			relationHotVO.setUserCodeList(teamUserCodes);
			relationHotVO.setUserCode(userCode);
			PageParam pageParam = new PageParam(Integer.valueOf(pageNo), Integer.valueOf(pageSize));
			PageBean pageBean = relationHotService.getTeamHotList(pageParam, relationHotVO);
			if (pageBean == null) {
				log.error("RelationHotController.getTeamHideHotList：查询班组隐藏热点列表失败");
				throw BusinessException.build("RELATION_19002", "查询班组隐藏热点列表");
			}
			List<RelationTeamHideHotModel> recordList = pageBean.getRecordList();
			JSONArray ja = new JSONArray();
			for (RelationTeamHideHotModel model : recordList) {
...
//					}
			...
			body.put("total", pageBean.getTotalCount());
			body.put("hotList", ja);
			resultJSON = new ResultJSON("COMMON_200", "查询班组隐藏热点列表");
			resultJSON.setBody(body);
			
			//方法出去
			log.info("RelationHotController.getTeamHideHotList:查询班组隐藏热点列表成功");
		} catch (BusinessException e) {
			throw e;
		} catch (Exception e) {
			log.error("RelationHotController.getTeamHideHotList", e);
			throw BusinessException.build("COMMON_400");
		}
	...
	public ResultJSON getPortalHotList(@RequireValid GetPortalHotListVO param) throws BusinessException {
		...
		try {
			//方法进入
			log.info("RelationHotController.getPortalHotList:查询门户展示中热点列表");
			...
			if (list == null) {
				log.error("RelationHotController.getPortalHotList:查询门户展示中热点列表失败");
			...
			//方法出来
			log.info("RelationHotController.getPortalHotList:查询门户展示中热点列表成功");
		} catch (BusinessException e) {
			throw e;
		} catch (Exception e) {
			log.error("RelationHotController.getPortalHotList", e);
			throw BusinessException.build("COMMON_400");
		}
	 ...
	 * @return: ResultJSON    返回类型
	 */
	@RequestMapping(value="getPortalFrontHotList",method=RequestMethod.POST)
	@ResponseBody
	public ResultJSON getPortalFrontHotList(@RequireValid GetPortalFrontHotListVO param) throws BusinessException {
		
		//返回值
		ResultJSON resultJSON = null;
		
		try {
			log.info("RelationHotController.getPortalFrontHotList:查询门户首页展示热点列表");
			
			//获取参数
			String qOrgCode = StringTools.formatToString(param.getOrgCode()); //门户编号
			String showSize = StringTools.formatToString(param.getShowSize());//显示条数
			String userCode = StringTools.formatToString(param.getUserCode());//用户编号
			//封装参数
			RelationHotVO relationHotVO = new RelationHotVO();
			relationHotVO.setOrgCode(qOrgCode);
			relationHotVO.setShowSize(Integer.valueOf(showSize));
			relationHotVO.setUserCode(userCode);
			//查询
			List<RelationPortalFrontHotModel> list = relationHotService.getMHPortalFrontHotList(relationHotVO);
			if (list == null) {
				log.error("RelationHotController.getPortalFrontHotList：查询门户首页展示热点列表失败");
				throw BusinessException.build("RELATION_19002", "查询门户首页展示热点列表");
			}
			//拼接返回参数
			JSONArray ja = new JSONArray();
			for (RelationPortalFrontHotModel model : list) {
				JSONObject jo = new JSONObject();
				jo.put("hotCode", StringTools.formatToString(model.getHotCode()));
				jo.put("subjectCode", StringTools.formatToString(model.getSubjectCode()));
				jo.put("subjectClass", StringTools.formatToString(model.getSubjectClass()));
				...
				String sourceOrgCode = "";
				String sourceTeamCode = StringTools.formatToString(model.getTeamCode());
				...
				jo.put("userCode", StringTools.formatToString(model.getUserCode()));
				jo.put("subjectDepend", StringTools.formatToString(model.getSubjectDepend()));
				jo.put("teamCode", sourceTeamCode);
				jo.put("orgCode", sourceOrgCode);
				jo.put("subjectDigest", model.getSubjectDigest());
				ja.add(jo);
			}
			JSONObject body = new JSONObject();
			body.put("hotList", ja);
			resultJSON = new ResultJSON("COMMON_200", "查询门户首页展示热点列表成功");
			resultJSON.setBody(body);
			
			log.info("RelationHotController.getPortalFrontHotList:查询门户首页展示热点列表成功");
		} catch (BusinessException e) {
			throw e;
		} catch (Exception e) {
			log.error("RelationHotController.getPortalFrontHotList", e);
			throw BusinessException.build("COMMON_400");
		}
	...
	public ResultJSON updateTeamHotStatus(@RequireValid UpdateTeamHotStatusVO param) throws BusinessException {
		...
		ResultJSON resultJSON = null;
		try {
			log.info("RelationHotController.updateTeamHotStatus:隐藏/显示班组热点");
			...
			if(sysUserInfo==null || sysUserInfo.getManageOrgInfo()==null||StringTools.isEmpty(sysUserInfo.getManageOrgInfo().getSysOrgCode())){
				log.error("RelationHotController.updateTeamHotStatus：用户或组织没有获取到userCode:"+param.getUserCode());
			...
			if (!updateFlag) {
				...
				log.info("RelationHotController.updateTeamHotStatus:隐藏/显示班组热点成功");
			}
			
		} catch (BusinessException e) {
			throw e;
		} catch (Exception e) {
			log.error("RelationHotController.updateTeamHotStatus", e);
			throw BusinessException.build("COMMON_400");
		}
