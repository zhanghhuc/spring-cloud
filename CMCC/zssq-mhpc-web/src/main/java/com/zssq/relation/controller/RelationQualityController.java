		...
		try {
			...
			if (null != recordList && !recordList.isEmpty()) {
				...
				for (int i = 0; i < recordSize; i++) {
					...
					item.put("subjectDepend", StringTools.formatToString(relationQualityModel.getSubjectDepend()));//内容从属关系
					item.put("userCode", StringTools.formatToString(relationQualityModel.getUserCode()));// 发布人编号
					item.put("teamCode", StringTools.formatToString(relationQualityModel.getTeamCode()));// 班组编号
