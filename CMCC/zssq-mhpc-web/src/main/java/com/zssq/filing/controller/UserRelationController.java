	...
	public ResultJSON concernsList(@RequireValid ListConVo param) throws BusinessException {
		...
		try {
			...
			for (int i = 0; i < myConcerns.size(); i++) {
				...
				myConcern.put("level", StringTools.formatToString(expAccount.getCurrentLevel()));//等级
...
//				statisticService.addRecord(statisticCommon);
	...
	public ResultJSON blackAdd(@RequireValid AddBlackVo param) throws BusinessException {
		...
		try {
			...
			String blackUserCode = param.getBlackUserCode();
			boolean isSuccess = userRelationService.addBlack(userCode, blackUserCode);
			//取消订阅博客
			BlogThirdVO blogThirdVO = new BlogThirdVO();
			blogThirdVO.setUserCode(userCode);	
			blogThirdVO.setSubClass(BlogConstants.BLOG_SUB_USER);
			blogThirdVO.setSubUserCode(blackUserCode);
			blogThirdService.deleteSubscribe(blogThirdVO);
			//取消订阅微博
			mblogThridService.updateUserSubscribe(userCode,blackUserCode);
	...
	public ResultJSON state(@RequireValid UserStateVo param) throws BusinessException {
		...
		try {
			...
			boolean shieldStatus = userRelationService.checkShield(param.getOtherCode());
			if(shieldStatus){
				body.put("isShielded", FilingConstants.BOOLEAN_TRUE);
			}else{
				body.put("isShielded", FilingConstants.BOOLEAN_FALSE);
			}
			message = PropertiesUtil.getMessage(FilingConstants.RETURNCODE_FILING_200);
			rj.setReturnCode(message.getCode());
	...
	public ResultJSON companyView(@RequireValid CompanyViewVo param) throws BusinessException{  
	    ...
	    try {
	    	...
	    	for (int i = 0; i < length; i++) {
	    		...
	    		orgInfo = sysOrgService.selectByCode(viewList.get(i).getOrgCode());
	    		if(orgInfo != null){
	    			info.put("orgName", StringTools.formatToString(orgInfo.getSysOrgFullname()));
	    			info.put("srcOrgCode", StringTools.formatToString(orgInfo.getSrcCode()));
	    			info.put("orgType", StringTools.formatToString(orgInfo.getSysOrgType()));
	    		}else{
	    			info.put("orgName", "");
	    			info.put("srcOrgCode", "");
	    			info.put("orgType", "");
