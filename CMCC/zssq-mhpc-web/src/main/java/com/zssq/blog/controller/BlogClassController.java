...
public class BlogClassController {
	...
	public ResultJSON createUserBlogClass(@RequireValid CreateUserBlogClassVO createUserBlogClassVO) throws BusinessException {
		...
		try {
			...
			// 敏感词过滤
            JSONArray jsonArray = solrQueryService.CheckSentence(sysUserInfo.getManageOrgInfo().getSysOrgCode(), createUserBlogClassVO.getClassName(),
	...
	public ResultJSON createTeamBlogClass(@RequireValid CreateTeamBlogClassVO createTeamBlogClassVO) throws BusinessException {
		...
		try {
			...
			// 敏感词过滤
            JSONArray jsonArray = solrQueryService.CheckSentence(teamInfo.getOrgCode(), createTeamBlogClassVO.getClassName(),
                    SolrCoreConstants.SENSITIVE_WORD_CORE);
            if( jsonArray.size() > 0 ){
            	result = new ResultJSON("COMMON_999");
    			JSONObject body = new JSONObject();
