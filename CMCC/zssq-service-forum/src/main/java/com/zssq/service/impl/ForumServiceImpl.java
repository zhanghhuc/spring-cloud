...
public class ForumServiceImpl extends CommonService implements IForumService {
	...
	@Override
	public int countReply(ForumCommentReply record) throws BusinessException {
		...
		try {
			...
			criteria.andIsDeleteEqualTo(ForumConstants.NO);
			// 是否隐藏:0-否
			criteria.andIsHideEqualTo(ForumConstants.NO);
			// 租户标识
			criteria.andTenantCodeEqualTo(record.getTenantCode());
			//主贴code
			criteria.andTopicCodeEqualTo(record.getTopicCode());
			//跟帖code
			criteria.andTopicFollowCodeEqualTo(record.getTopicFollowCode());
			
			return forumCommentReplyMapper.countByExample(example);
		} catch (Exception e) {
			log.error("ForumServiceImpl.countReply", e);
			throw BusinessException.build("COMMON_400");
		}
