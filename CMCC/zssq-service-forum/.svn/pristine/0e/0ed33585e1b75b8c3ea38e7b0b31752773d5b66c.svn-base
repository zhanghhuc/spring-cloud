...
public class ForumThirdServiceImpl extends CommonService implements IForumThirdService {
	...
	public List<ForumTopics> sumupTeamForum(String teamCode, int startNum, int endNum) throws BusinessException {
		try {
			List<ForumTopics> parentList = new ArrayList<ForumTopics>();
			...
			// 查询主帖列表
			List<ForumTopics> topicList = forumTopicsMapper.queryTopicByBelong(paramMap);
			for (int i = 0; i < topicList.size(); i++) {
				ForumTopics ft = topicList.get(i);
				int followCount = ft.getReplyCount();
				if (followCount > 0 && ft.getIsHide() == ForumConstants.NO) {
					// 查询跟帖列表
					String topicCode = ft.getForumTopicsCode();// 主帖CODE
					
					ForumTopicsFollowExample fExample = new ForumTopicsFollowExample();
					ForumTopicsFollowExample.Criteria fCriteria = fExample.createCriteria();
					
					fCriteria.andIsDeleteEqualTo(ForumConstants.NO);
					fCriteria.andIsDisableEqualTo(ForumConstants.NO);
					fCriteria.andTopicCodeEqualTo(topicCode);
					
					fExample.setOrderByClause("id asc");
					
					List<ForumTopicsFollow> followList = forumTopicsFollowMapper.selectByExampleWithBLOBs(fExample);
					
					List<ForumTopicsFollow> childList = new ArrayList<ForumTopicsFollow>();
					for (int j = 0; j < followList.size(); j++) {
						ForumTopicsFollow ftf = followList.get(j);
						Integer commentReplyCount = ftf.getFollowCount();
						if (commentReplyCount != null && commentReplyCount > 0
								&& ftf.getIsHide() == ForumConstants.NO) {
							// 查询评论回复列表
							String followCode = ftf.getForumTopicsFollowCode();
							
							ForumCommentReplyExample cExample = new ForumCommentReplyExample();
							ForumCommentReplyExample.Criteria cCriteria = cExample.createCriteria();
							
							cCriteria.andIsDeleteEqualTo(ForumConstants.NO);
							cCriteria.andIsDisableEqualTo(ForumConstants.NO);
							cCriteria.andTopicCodeEqualTo(topicCode);
							cCriteria.andTopicFollowCodeEqualTo(followCode);
							
							cExample.setOrderByClause("id desc");
							
							List<ForumCommentReply> commentReplyList = forumCommentReplyMapper.selectByExampleWithBLOBs(cExample);
							ftf.setCommentReplyList(commentReplyList);
						}
						childList.add(ftf);
					}
					ft.setFollowList(childList);
			...
			log.error("ForumThirdServiceImpl.sumupTeamForum", e);
			throw BusinessException.build("COMMON_400");
		}
	}
