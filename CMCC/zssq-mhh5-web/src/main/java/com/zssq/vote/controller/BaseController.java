...
/**
 * 投票、荣誉、活动、论坛、举报模块的BaseController
...
public class BaseController {
	...
	protected void noticeAction(String manageOrgCode, String actionCode, String accountCode, Byte accountType) throws BusinessException {
		...
		producerTemplate.send(CreditConstants.CREDIT_INCREMENT, data);
	}
	...
	/**
	 * 获得某个机构的组织机构树
	 * @Function getOrgTreeByAnyOrgCodes
	 * @Description 
	 * @param orgType 组织机构类型：1或A：集团；2或B：省；3或C：市
	...
	/**
	 * 获取用户所在的班组CODE列表
	 * @Function getUserCodeTeamCodes
	 * @Description 
	 * @param userCode 用户CODE
	 * @return
	 * @throws BusinessException
	 */
	protected List<String> getUserCodeTeamCodes(String userCode) throws BusinessException {
		List<String> returnList = new ArrayList<String>();
		List<TeamInfo> list = teamMemberService.selectTeamByUser(userCode);
		if (list != null) {
			for (TeamInfo teamInfo : list) {
				returnList.add(teamInfo.getTeamCode());
			}
		}
		return returnList;
	}
	...
	/**
	 * 获取班组长CODE列表
	 * @Function selectTeamLeaderCodesByTeamCode
	 * @Description 
	 * @param teamCode 班组CODE
	 * @return
	 * @throws BusinessException
	 */
	protected List<String> selectTeamLeaderCodesByTeamCode(String teamCode) throws BusinessException {
		return teamMemberService.selectTeamLeaders(teamCode);
	}
	/**
	 * 根据orgCode查询组织机构详情
	 * @Function getOrgInfoByCode
	 * @Description 
	 * @param orgCode 组织机构编码
	 * @return
	 * @throws BusinessException
	 */
	protected SysOrgInfo getOrgInfoByCode(String orgCode) throws BusinessException {
		SysOrgInfo sysOrgInfo = sysOrgService.selectByCode(orgCode);
//		if (sysOrgInfo == null) {
//			throw BusinessException.build("VOTE_18002", "组织机构信息");
//		}
		return sysOrgInfo;
	}
						...
						try {
							...
							if(null != record){
								...
								if (HonorConstants.HONOREE_TYPE_2.equals(record.getHonoreeType())) {
									// 授予班组荣誉
									TeamInfo teamInfo = getTeamInfoByCode(honorAwardRecord.getHonoreeCode());
