...
import com.zssq.relation.vo.GetDynamicSourceVO;
...
public class RelationDynamicController {
	...
	/**
	 * 
	 * @Title: getDynamicSource  
	 * @Description: 查询动态来源
	 * @param getDynamicSourceVO
	 * @throws BusinessException    参数  
	 * @return: ResultJSON    返回类型
	 */
	@RequestMapping(value="getDynamicSource",method=RequestMethod.POST)
	@ResponseBody
	public ResultJSON getDynamicSource(@RequireValid GetDynamicSourceVO getDynamicSourceVO) throws BusinessException {
		// 返回值
		ResultJSON result = null;
		
		try {
			log.info("RelationDynamicController.getDynamicSource：查询动态来源");
			...
			// 获取参数
			String dynamicCode = getDynamicSourceVO.getDynamicCode(); // 动态编号
			String dynamicUserCode = getDynamicSourceVO.getDynamicUserCode(); // 动态所属人编号
			String userCode = getDynamicSourceVO.getUserCode(); // 人员编号
			
			// 查询两人关系
			UserState userState = iUserRelationService.selectUserState(userCode, dynamicUserCode);
			if (userState == null || userState.getIsBlack() == null) {
				log.error("RelationDynamicController.getDynamicSource：获取不到人员关系信息");
				throw BusinessException.build("COMMON_400");
			}
			if (FilingConstants.BOOLEAN_TRUE.equals(userState.getIsConcerns())) {
				total = 1;
				JSONObject jo = new JSONObject();
				jo.put("sourceType", StringTools.formatToString(RelationConstants.RELATION_DYNAMIC_DEPEND_USER));
				jo.put("sourceCode", StringTools.formatToString(dynamicUserCode));
				// 获取人员信息
				SysUserInfo sysUserInfo = iSysUserService.selectByCode(dynamicUserCode);
				jo.put("sourceName", sysUserInfo == null ? "" : StringTools.formatToString(sysUserInfo.getUserName()));
				ja.add(jo);
			}
			...
			// 查询
			List<String> teamCodes = relationDynamicService.getDynamicTeamCodes(relationDynamicVO);
			if (teamCodes == null) {
				log.error("RelationDynamicController.getDynamicSource：查询失败");
				throw BusinessException.build("RELATION_19002", "查询");
			}
			
			// 返回值赋值
			total += teamCodes.size();
			for (String teamCode : teamCodes) {
				JSONObject jo = new JSONObject();
				jo.put("sourceType", StringTools.formatToString(RelationConstants.RELATION_DYNAMIC_DEPEND_TEAM));
				jo.put("sourceCode", StringTools.formatToString(teamCode));
				// 获取班组信息
				TeamInfo teamInfo = iTeamInfoService.selectByCode(teamCode);
				jo.put("sourceName", teamInfo == null ? "" : StringTools.formatToString(teamInfo.getTeamName()));
				ja.add(jo);
			}
			...
			body.put("sourceList", ja);
			result.setBody(body);
			
			log.info("RelationDynamicController.getDynamicSource：查询动态来源成功");
		} catch (BusinessException e) {
			throw e;
		} catch (Exception e) {
			log.error("RelationDynamicController.getDynamicSource", e);
			throw BusinessException.build("COMMON_400");
		}
