...
public class TeamController {
	...
	public ResultJSON headInfo(@RequireValid HeadInfoVo param) throws BusinessException{  
	    ...
	    try {
	    	...
	    	if(teamInfo.getIsLeader() != null){//这个班组是我加入的班组,记录访问
	    		teamInfoService.addTeamZone(param.getUserCode(), param.getTeamCode());
	    	}
	    	JSONObject body = new JSONObject();
	    	String teamIcon = authConfig.getTeamIcon();
...
//	    	body.put("isLeader", StringTools.formatToString(teamInfo.getIsLeader()));//当前登录人是否是班组长
	...
	/**
	 * @Function zone
	 * @Description 点击下边栏的班组空间,返回需要放的班组编码
	 * @param request
	 * @return
	 * @throws BusinessException
	 */
	@ResponseBody
	@RequestMapping("/zone")
	public ResultJSON zone(HttpServletRequest request) throws BusinessException{  
	    Message m = null;
	    ResultJSON rj = new ResultJSON();
	    try {
	    	String teamCode = teamInfoService.getTeamZone(request.getParameter("userCode"));
	    	JSONObject body = new JSONObject();
	    	if(teamCode == null){
	    		List<TeamInfo> teamList = teamInfoService.getMyTeamList(request.getParameter("userCode"));
	    		if(teamList.size() > 0){
	    			body.put("teamCode", teamList.get(0).getTeamCode());
	    		}else{
	    			m = PropertiesUtil.getMessage(TeamConstants.RETURNCODE_TEAM_15002);
	    	    	rj.setReturnCode(m.getCode());
	    	    	rj.setReturnTip("你还未加入任何班组");
	    	    	rj.setBody(body);
	    	    	return rj;
	    		}
	    	}else{
	    	...
	    	m = PropertiesUtil.getMessage(TeamConstants.RETURNCODE_TEAM_200);
	    	rj.setReturnCode(m.getCode());
	    	rj.setReturnTip(String.format(m.getTip(),"拿到记录的访问班组"));
	    	rj.setBody(body);
	    } catch (Exception e) {
			m = PropertiesUtil.getMessage(TeamConstants.RETURNCODE_TEAM_15002);
			log.error("班组空间tab跳转异常", e);
			throw new BusinessException(m.getCode(),String.format(m.getTip(),"班组空间tab跳转"));
		}
	    return rj;
	}
...
//				statisticService.addRecord(statisticCommon);
	    	...
	    	rj.setReturnCode(m.getCode());
	    	rj.setReturnTip(String.format(m.getTip(),"移除班组粉丝"));
	    	rj.setBody(body);
		} catch (Exception e) {
			m = PropertiesUtil.getMessage(TeamConstants.RETURNCODE_TEAM_15002);
			log.error("移除班组粉丝异常", e);
			throw new BusinessException(m.getCode(),String.format(m.getTip(),"移除班组粉丝"));
		}
	    return rj;
	}
	    ...
	    try {
	    	...
	    	for (int i = 0; i < fans.size(); i++) {
	    		...
	    		if(userInfo != null){
	    			...
	    			info.put("intro", StringTools.formatToString(userInfo.getIntroduce()));//简介
	    	...
	    	for (int i = 0; i < fans.size(); i++) {
	    		...
	    		if(userInfo != null){
	    			...
	    			for (int j = 0; j < honorList.size() && j < 5; j++) {
						...
						honorInfo.put("honorCode", StringTools.formatToString(honorList.get(j).getHonorCode()));
