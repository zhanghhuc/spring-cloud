...
public class IntegralAccountDetailController {
	...
	/**
	 * 
	 * @Title: listIntegralAccountDetail  
	 * @Description: 积分明细查询
	 * @param @param param
	 * @param @return
				...
				for (IntegralAccountDetail recordTemp : recordList) {
					...
					if(recordTemp != null){
						...
						IntegralAction integralAction = 
								integralActionService.selectByActionCode(recordTemp.getActionCode());
						if(integralAction != null){
							actionRemark = integralAction.getActionRemark();
						}
						jsonObject.put("actionRemark", 
								StringTools.formatToString(actionRemark));
						jsonObject.put("currentValue", 
								StringTools.formatToString(recordTemp.getCurrentValue()));           // 当前增减值
						jsonObject.put("currentBalance", 
								StringTools.formatToString(recordTemp.getCurrentBalance()));         // 当前增减后余额
						jsonObject.put("createTime", 
								StringTools.formatToString(recordTemp.getCreateTime()));             // 创建时间
