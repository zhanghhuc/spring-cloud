...
import com.zssq.filing.dao.mapper.SynFilterBaseMapper;
import com.zssq.filing.dao.mapper.SysSensitiveWordMapper;
...
import com.zssq.filing.pojo.SysSensitiveWord;
...
public class UserRelationServiceImpl implements UserRelationService {
	...
	@Autowired
	private SynFilterBaseMapper synFilterBaseMapper;
	@Autowired
	 ...
	 * 
	 * 迁移敏感词.
	 * @see com.zssq.filing.service.UserRelationService#transferSenstiveWord()
	 */
	@Override
	public boolean transferSenstiveWord() {
		// 每次处理记录数
		Integer executeSize = 500;
		// 记录起始行号
		Integer startRowNum = 0;
		try{
			// 获取源表记录数		
			Integer dataSize = synFilterBaseMapper.selectCount();	
			logger.info("----------敏感词迁移记录总数:"+dataSize+"----------");
			logger.info("----------敏感词迁移开始----------");
			while(startRowNum <= dataSize) {
				Map<String, Object> param = new HashMap<String, Object>();
				param.put("startRowNum", startRowNum);
				param.put("executeSize", executeSize);
				List<SynFilterBase>  synFilterBaseList= synFilterBaseMapper.selectPage(param);			
				
				List<SysSensitiveWord> sysSensitiveWordList = new ArrayList<SysSensitiveWord>();
				for(SynFilterBase synFilterBase : synFilterBaseList) {
					SysSensitiveWord sysSensitiveWord = new SysSensitiveWord();
					sysSensitiveWord.setSensitiveCode(UUIDHelper.getUUID());
					sysSensitiveWord.setSensitiveValue(synFilterBase.getWord());
					sysSensitiveWord.setIsEnable(new Byte("1"));//启用1
					sysSensitiveWord.setOrgCode("0");//基础存 "0"
					sysSensitiveWordList.add(sysSensitiveWord);
				}
				//批量新增
				sysSensitiveWordMapper.bathInsert(sysSensitiveWordList);
