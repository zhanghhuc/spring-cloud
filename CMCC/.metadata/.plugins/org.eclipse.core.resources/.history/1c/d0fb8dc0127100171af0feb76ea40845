...
public class ForumController extends BaseController {
		...
		try {
			...
			if (topic.getForumTopicsCode() != null) {
				...
				if (floorHost != null) {
					...
					body.put("orgCode", StringTools.formatToString(floorHost.getManageOrgInfo().getSysOrgCode()));
					body.put("orgName", StringTools.formatToString(floorHost.getManageOrgInfo().getSysOrgName()));
				} else {
			...
			if (list.size() > 0) {
				...
				for (ForumTopicsFollow forumTopicsFollow : list) {
					...
					if (followReplier != null) {
						...
						json.put("orgCode", StringTools.formatToString(followReplier.getManageOrgInfo().getSysOrgCode()));
						json.put("orgName", StringTools.formatToString(followReplier.getManageOrgInfo().getSysOrgName()));
					} else {
					...
					if (ForumConstants.NO.equals(forumTopicsFollow.getIsHide())) {
						...
						String canbeComplaint = "1";
						if(forumTopicsFollow.getFollowerCode().equals(vo.getUserCode())){
							canbeComplaint = "0";
						}
						json.put("canbeComplaint", canbeComplaint);// 是否具有举报权限：0-否  1-是
					} else {
	...
	public ResultJSON getFollowTopicReplyList(@RequireValid GetTopicFollowReplyListVo vo) throws BusinessException {
		try {
			...
			int total = forumService.countReply(record);
			JSONArray array = new JSONArray();
			if (list.size() > 0) {
				ForumPlatesMember member = forumAuthService.queryUserForumAuthority(tenantCode, topic.getForumPlatesCode(), vo.getUserCode());
				...
				for (ForumCommentReply reply : list) {
					...
					if (ForumConstants.NO.equals(reply.getIsHide())) {
						...
						json.put("canbeShield", StringTools.formatToString(member.getCanbeShield()));// 是否具有屏蔽权限：0-否  1-是
						String canbeComplaint = "1";
						if(reply.getReplierCode().equals(vo.getUserCode())){
							canbeComplaint = "0";
						}
						json.put("canbeComplaint", canbeComplaint);// 是否具有举报权限：0-否  1-是
					} else {
			...
			body.put("total", StringTools.formatToString(total));
