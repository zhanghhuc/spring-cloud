    ...
    */  
	...
	public PageBean gotoCommentListByPage(CommentInfoQO qo) throws BusinessException {
		...
		try{
			...
			// 获取评论列表
			List<MblogComment> dataList = new ArrayList<MblogComment>();
			Map<String,Object> paramsMap = new HashMap<String,Object>();
			paramsMap.put("lineTime", qo.getLineTime());
			paramsMap.put("mblogCode",qo.getMblogCode());
			paramsMap.put("limitStart", qo.getLimitStart());
			...
			if(0 == qo.getPageNo()){
				...
				paramsMap1.put("commentCode", qo.getCommentCode());
				MblogComment comment = mblogCommentMapper.getDetailByCode(paramsMap1);
			...
			List<MblogComment> comList = mblogCommentMapper.queryCommentListByPage(paramsMap);
			dataList.addAll(comList);
			// 查询数据
