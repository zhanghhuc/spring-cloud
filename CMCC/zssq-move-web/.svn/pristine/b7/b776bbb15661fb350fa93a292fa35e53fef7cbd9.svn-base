...
//			int DB2CommentCount = blogCommentService.getDB2CommentCount();
			int DB2CommentCount = blogCommentService.getSourceMySQLCommentCount();
			if (DB2CommentCount <= 0) {
				body.put("message", "查询源数据库中评论数据总量时出错，查询出的个数为：" + DB2CommentCount);
				throw new Exception();
			}
...
//		int DB2CommentCount = blogCommentService.getDB2CommentCount();
		int DB2CommentCount = blogCommentService.getSourceMySQLCommentCount();
		if (DB2CommentCount <= 0) {
			System.err.println(this.getClass() + "查询源数据库中评论数据总量时出错，查询出的个数为：" + DB2CommentCount);
			throw new Exception();
		}
...
//			List<DB2BlogReply> DB2ReplyList = blogCommentService.getDB2CommentList(pageNo, pageSize);
			List<DB2BlogReply> DB2ReplyList = blogCommentService.getSourceMySQLCommentList(pageNo, pageSize);
