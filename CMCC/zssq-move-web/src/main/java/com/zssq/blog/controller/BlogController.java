...
//			int DB2BlogCount = blogService.getDB2BlogCount();
			int DB2BlogCount = blogService.getSourceMySQLBlogCount();
			if (DB2BlogCount <= 0) {
				body.put("message", "查询源数据库中博客数据总量时出错，查询出的个数为：" + DB2BlogCount);
				throw new Exception();
			}
...
//		int DB2BlogCount = blogService.getDB2BlogCount();
		int DB2BlogCount = blogService.getSourceMySQLBlogCount();
		if (DB2BlogCount <= 0) {
			System.err.println(this.getClass() + "查询源数据库中博客数据总量时出错，查询出的个数为：" + DB2BlogCount);
			throw new Exception();
		}
...
//			List<DB2Blog> DB2BlogList = blogService.getDB2BlogList(pageNo, pageSize);
			List<DB2Blog> DB2BlogList = blogService.getSourceMySQLBlogList(pageNo, pageSize);
