	...
	public ResultJSON getAwardRecordDetail(@RequireValid GetAwardRecordDetailVo inVo) throws BusinessException {
		try {
			...
			if (null != har) {
				...
				// 根据 replierCode 查询 replierName、replierUrl、replierOrg
				SysUserInfo awarderInfo = getUserInfo(har.getAwarderCode());
				if(awarderInfo != null){
					jsonObject.put("awarderOrg", StringTools.formatToString(awarderInfo.getManageOrgInfo().getSysOrgName()));
				}else{
					jsonObject.put("awarderOrg", "");
				}
