...
import com.zssq.msg.vo.BoardMsgVO;
...
public class MsgSystemController {
	...
	@Autowired
	private MessagerBoradService boardService;
	...
	public ResultJSON addMsgInfo(@RequireValid AddSystemVO vo) throws BusinessException{
		...
		sysModel.setOrgCode(userInfo.getOrgCode());
		sysModel.setShowOrgCode(userInfo.getOrgCode());
	...
	@ResponseBody
	public ResultJSON getBoardMsgList(@RequireValid BoardMsgVO vo) throws BusinessException{
		ResultJSON resJson=new ResultJSON();
		Message m = null;
		...
		try {
			...
			PageBean pageBean=boardService.ManagementBoardMsg(Integer.parseInt(vo.getPageSize()), Integer.parseInt(vo.getPageNo()), vo.getOrgCode());	

			List<UserMsgBoard> boardList= pageBean.getRecordList();
			List<String> userCodeList=new ArrayList<>();
			
			for(UserMsgBoard temp:boardList){
				userCodeList.add(temp.getUserCode());
			}

			Map<String, Object>  userMap=userService.selectMapByCodes(userCodeList);
			
			for(UserMsgBoard board:boardList){
				SysUserInfo userInfo=(SysUserInfo) userMap.get(board.getUserCode());
				
				json.put("userName",userInfo.getUserName());
				json.put("orgName",userInfo.getManageOrgInfo().getSysOrgName());
				json.put("createTime",board.getCreateTime());
				json.put("content",board.getContent());
				json.put("userCode",board.getUserCode());
				json.put("receiveUserCode",board.getUserCode());
				json.put("msgCode",board.getMessageBoardCode());
				jsonList.add(json);
				json=new JSONObject();
			}
			...
			logger.error("获取留言表-getMsgBoardList():"+JSONObject.toJSONString(vo),e);
