		...
		try {
			...
			if(CollectionUtils.isNotEmpty(smapUserInfos)) {
				...
				for(UserInfo smapUserInfo : smapUserInfos) {
					...
					sysUserInfo.setModifyTime(DateUtils.getFormatDateLong());			// 更新时间
...
//			log.info("组织清洗，删除节点：删除员工数量：" + delUserCount);
			...
			if(StringUtils.isNotBlank(orgJson)) {
				log.debug("SysOrgInfoServiceImpl.selectByCode：从缓存中获取到了组织机构信息 " + orgCode);
				SysOrgInfo sysOrgInfo = JSON.parseObject(orgJson, SysOrgInfo.class);
				sysOrgInfo.setIsEnable(Byte.parseByte(AuthConstants.LOCAL_ORGSTATUS_DELETE));
				orgJson = JSON.toJSONString(sysOrgInfo);
				jedisClusterService.set(sysOrgInfo.getSysOrgCode(), orgJson);
			}
	...
	public void setIsEnable(String orgCode, Byte isEnable, String tenantCode) throws BusinessException {
		...
		try {
			...
			if(StringUtils.isNotBlank(orgJson)) {
				log.debug("SysOrgInfoServiceImpl.selectByCode：从缓存中获取到了组织机构信息 " + orgCode);
				SysOrgInfo sysOrgInfo = JSON.parseObject(orgJson, SysOrgInfo.class);
				sysOrgInfo.setIsEnable(isEnable);
				orgJson = JSON.toJSONString(sysOrgInfo);
				jedisClusterService.set(sysOrgInfo.getSysOrgCode(), orgJson);
			}
...
//			}
	...
	public void refreshOrgCache(String orgCode) throws BusinessException {
		try {
			jedisClusterService.del(orgCode);
			selectByCode(orgCode);
			
			log.info("刷新组织缓存成功,orgCode = " + orgCode);
		} catch (Exception e) {
			String errMsg = "刷新组织缓存出错,orgCode = " + orgCode;
			log.error(errMsg, e);
			throw new BusinessException(errMsg, e);
		}
