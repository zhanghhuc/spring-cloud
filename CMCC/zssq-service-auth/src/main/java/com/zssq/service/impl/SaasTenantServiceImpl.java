...
//			searchParam.setIsEnable(AuthConstants.BOOLEAN_TRUE);
			// 根据登录帐号，从管理员信息表中获取Saas帐号，判断密码是否匹配
			SysAdminInfo sysAdminInfo = saasTenantMapper.selectByAccount(searchParam);
			if(!sysAdminInfo.getIsEnable().equals(AuthConstants.BOOLEAN_TRUE)){//禁用
				return sysAdminInfo;
			}
	...
	public boolean setTenantState(String tenants,String isEnable) throws BusinessException {
		try {
			...
			saasTenantInfoMapper.setTenantState(record);
			
			//启禁用 超管
			SysAdminInfo adminInfo = new SysAdminInfo();
			adminInfo.setIsEnable(Byte.parseByte(isEnable));
			adminInfo.setTenantCodes(codes);
			return saasTenantMapper.setAdminState(adminInfo) > 0;
		} catch (Exception e) {
			log.error("租户启禁用出错", e);
